    ...
    try {
        ...
        if (sel.baseNode) {
            ...
            if (baseNode.nodeType == Node.TEXT_NODE) {
                void(0);
                if (baseOffset > 0) {
                    console.log("getSelectionRect:baseOffset:" + baseOffset);
					console.log("getSelectionRect:txt.length:" + txt.length);
					console.log("getSelectionRect:baseNode.textContent:" +baseNode.textContent);
					console.log("getSelectionRect:sel.baseNode:" + sel.baseNode);
					console.log("getSelectionRect:sel.baseNode.childNodes lenght:" + sel.baseNode.childNodes.length);
					for (i = 0; i < sel.baseNode.childNodes.length; ++ i) {
						console.log("getSelectionRect:sel.baseNode.childNodes:child " + i + " " + sel.baseNode.childNodes[i]);
					}
					console.log("getSelectionRect:sel.rangeCount:" + sel.rangeCount);
					console.log("getSelectionRect:baseNode.parentElement"+baseNode.parentElement);
                    void(0);
					if (baseOffset == txt.length) {
						console.log("getSelectionRect:baseOffset == txt.length");
						...
						rect = tempDiv.getBoundingClientRect();
						...
						console.log("getSelectionRect:rect:top=" + rect.top + ", rect.bottom=" + rect.bottom + ", baseOffset=" + baseOffset + ", txtLen=" + txtLen);
                    ...
                    void(0);
                    console.log("getSelectionRect:TEXT_NODE txtLen > 0. txtLen="+txtLen);
                    charRange = document.createRange();
                    ...
                    void(0);
                    console.log("getSelectionRect:baseOffset=0 and txtLen=0");
                    console.log("getSelectionRect:baseNode.parentElement"+baseNode.parentElement);
                ...
                console.log("getSelectionRect:rect.left:"+rect.left+",rect.top:"+rect.top+",rect.right:"+rect.right+",rect.bottom:"+rect.bottom+",window.scrollX:"+window.scrollX+",window.scrollY:"+window.scrollY);
                rect = {left : rect.left+window.scrollX, top : rect.top + window.scrollY, right : rect.right + window.scrollX, bottom : rect.bottom + window.scrollY};
